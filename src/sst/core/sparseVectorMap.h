...
// Give an iterator over std::pair's returns an iterator over those pairs second elements.
template<typename PairIter>
class PairFirstsIterator {
    PairIter current;
public:
    using value_type = std::remove_cv_t<std::remove_reference_t<decltype(current->second)>>;
    using reference  = decltype(current->second);
    ...
    explicit PairFirstsIterator(PairIter it) : current(it) {}

    reference operator*() const { return current->second; }
    pointer operator->() const { return &(current->second); }

    PairFirstsIterator& operator++() { ++current; return *this; }
    PairFirstsIterator  operator++(int) { PairFirstsIterator tmp = *this; ++current; return tmp; }

    bool operator==(const PairFirstsIterator& other) const { return current == other.current; }
    bool operator!=(const PairFirstsIterator& other) const { return current != other.current; }
};

// This is a SparseVectorMap that stores its data as (key,value) pairs.
// Normally when querying an element in the SparseVectorMap we call the values
// key() method as we bisect. This becomes an issue if we want to delete the
// underlying elements without having to resize and reshuffle the underlying
// map.  This version of the map stores the keys separately so that we can
// continue to use them even after an element has been deleted.
template <typename keyT, typename classTPtr>
class SplitKeySparseVectorMap
{
private:
    friend class SST::Core::Serialization::serialize_impl<SplitKeySparseVectorMap<keyT, classTPtr>>;
    static_assert(std::is_pointer<classTPtr>::value, "SplitKeySparseVectorMap expects to hold pointer types");
    ...
    /**
       ...
       already in the map, it will return -1.
     */
    using data_pair_type = std::pair<keyT, classTPtr>;
    using data_type = std::vector<data_pair_type>;
    data_type data;
    int                  binary_search_insert(keyT id) const
    {
        // For insert, we've found the right place when id < n && id >
        ...
        // Check to see if it goes top or bottom
        if ( size == 0 ) return 0;
        if ( id > data[size - 1].first ) return size;
        if ( id < data[0].first ) return 0;
        ...
        while ( bottom <= top ) {
            middle = bottom + (top - bottom) / 2;
            if ( id == data[middle].first ) return -1; // Already in map
            if ( id < data[middle].first ) {
                // May be the right place, need to see if id is greater
                // than the one before the current middle.  If so, then we
                // have the right place.
                if ( id > data[middle - 1].first )
                    return middle;
                else {
                    top = middle - 1;
                }
...
public:
    ...
    /**
       ...
       data in.
    */
    SplitKeySparseVectorMap(std::vector<classTPtr>& new_data, bool sorted = false)
    {
        data.swap(new_data);
        if ( !sorted ) {
            sort();
    ...
    {
        std::sort(data.begin(), data.end(),
            [](const data_pair_type lhs, const data_pair_type rhs) -> bool { return lhs.first < rhs.first; });
    }

    template <typename Compare = std::function<bool(const classTPtr, const classTPtr)>>
    void sort(Compare comp)
    {
        std::sort(data.begin(), data.end(),
            [&](const data_pair_type lhs, const data_pair_type rhs) -> bool { return comp(lhs.second, rhs.second); });
    }

    using raw_iterator              = typename data_type::iterator;
    using raw_const_iterator        = typename data_type::const_iterator;
    using pairsecond_iterator       = PairFirstsIterator<typename data_type::iterator>;
    using pairsecond_const_iterator = PairFirstsIterator<typename data_type::const_iterator>;
    ...
    */
    classTPtr insert(classTPtr val)
    {
        int index = binary_search_insert(val->key());
        if ( index == -1 ) return data[binary_search_find(val->key())].second; // already in the map
        raw_iterator it = data.begin();
        it += index;
        data.insert(it, std::make_pair(val->key(), val));
    ...
    /**
       ...
       @return begin iterator to data vector
     */
    pairsecond_iterator begin() {
        return pairsecond_iterator(data.begin());
    }
    ...
    /**
       ...
       @return end iterator to data vector
     */
    pairsecond_iterator end() {
      return pairsecond_iterator(data.end());
    }
    ...
    /**
       ...
       @return const begin iterator to data vector
     */
    pairsecond_const_iterator begin() const {
      return pairsecond_const_iterator(data.begin());
    }
    ...
    /**
       ...
       @return const end iterator to data vector
     */
    pairsecond_const_iterator end() const {
      return pairsecond_const_iterator(data.end());
    }
    ...
    */
    const classTPtr operator[](keyT id) const
    {
        int index = binary_search_find(id);
        if ( index == -1 ) {
            // Need to error out
    ...
    */
    ...
    void filter(filterT& filt)
    {
        ...
        for ( size_t i = 0; i < data.size(); ++i ) {
            ...
            data[write_ptr] = std::make_pair(key, filt(data[i].second));
            if ( data[write_ptr].second != nullptr ) {
                // Make sure the keys match
                if ( UNLIKELY(data[write_ptr].first != key) ) {
                    // No recovering from this, just need to error out
                    throw bad_filtered_key_error(
                        "ERROR: Filter object passed to SparseVectorMap::filter returned an object that had a "
                        "different value of key() than what was passed into the filter.  The filter object must return "
...
template <typename keyT, typename classTPtr>
class serialize_impl<SST::SplitKeySparseVectorMap<keyT, classTPtr>>
{
public:
    void operator()(SST::SplitKeySparseVectorMap<keyT, classTPtr>& v, SST::Core::Serialization::serializer& ser, ser_opt_t options)
    {
        //SST_SER(v.data, options);
    }
};
